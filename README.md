...
Two credential-adjacent notes. First, a **chart-cache hit on a pinned version skips fetching — and therefore authentication — entirely** (pinned chart versions are immutable, so the cached content is identical; use `--no-cache` to force a re-fetch). A chart pinned by a semver CONSTRAINT (`^1.2.0`, `1.x`) is cached too, but only after the constraint has been resolved against the registry, so that hit does contact it — the mutable half of the question (which version does this constraint mean *now*) is asked every run, and only the immutable half (this version's content) comes from disk. Second, the render cache keys on **both** the credential source and the **name → URL mappings** it provides, so switching `--repo-creds` re-renders instead of answering from the previous mode's entries. That matters for the obvious workflow: render with `local`, then re-run with `cluster` to check ArgoCD's own credentials can produce the same manifests. Served from cache, the second run would answer "yes" without consulting the cluster at all, and the merge would fail on a repository ArgoCD cannot reach. The mappings are keyed separately because helm resolves a `repository: "@myrepo"` chart dependency through them, so a name pointed elsewhere changes what gets rendered even within one mode. Credentials themselves are never hashed - rotating a password changes no manifest.

The chart cache is scoped by credential source for the same reason, so a **mode switch really does re-fetch**: charts downloaded under `local` live in their own directory and cannot answer for `cluster`. Inside `cluster` mode the scope goes one level further — the cluster API server and ArgoCD namespace (the server, not the context name: a name is an alias local to one kubeconfig file, and two files can both call different clusters `prod`) — because two clusters read with the same flag are two credential sources, and verifying cluster B's credentials must not be answered by cluster A's download. The cost is one re-download per credential source for content that is byte-identical, which is what makes the verification real rather than reported.
...
What remains, within a single mode: a PINNED chart already downloaded is served without contacting the registry, so a credential that expired since the download stays invisible (a constraint-pinned one still resolves against the registry first, so an expired credential surfaces there). `--no-cache=charts` forces the download for apps that re-render anyway, and `--no-cache` forces everything. Local charts pulling helm dependencies always re-fetch on a re-render, since each render builds them in a fresh isolated helm home.
...
argocdf runs the tool itself and parses its report, so there is no shell pipeline, no `jq`, and none of the ways an adapter is usually written wrong (see the exit-code trap below). `kyverno apply` is pointed at the cluster argocdf is diffing automatically — it refuses to run rather than fall back to the ambient one. The directory is repo-relative and resolved per side, so a PR that changes a policy lints each side with its own version; a side where it is absent (the base of a PR adding the first policy) is skipped rather than failed — and reported as `[base] [lint-kyverno#1] not linted: no policies in "policies/kyverno"`, because an unlinted side leaves every finding one-sided, which otherwise reads as "introduced by this change" when it is really "pre-existing, newly detected". A directory missing from your working tree is warned about at startup too, so a typo does not read as "no findings" — and so is one that holds no file the tool would load (only a `.gitkeep`, say), which would otherwise produce a linter that checked nothing. One consequence is worth knowing: a change that touches **only** policy files selects no application, because selection matches changed files against each app's `source.path`. Nothing renders and no policy runs. To see a new policy's effect across your apps, change something under an app's path as well, or run argocdf against a branch that does. Order is fixed and report-visible: `--lint` commands, then kyverno, then conftest.
...
The exit code is the only health signal: stdout lines are always reported as warnings, and a spawn failure, timeout, or exit ≠ 0 adds one non-fatal `[lint#1] exit status 3: ...` warning line. Every warning argocdf writes itself opens with the linter's identity — the flag that configured it plus its position among the flags of that kind — and whether the bracket continues past it tells you what you are reading:

```
[target] [lint#1/disallow-latest-tag] Deployment/web: container images must be pinned to a tag
[base]   [lint-kyverno#1] not linted: no policies in "policies/kyverno"
[target] [lint#2] timeout after 10s
```

A bracket carrying a policy is a **finding**, and the rest of the line comes from the tool. A bracket that stops at the linter is **argocdf reporting on the linter itself** — a timeout, a crash, a side with no policies. The ordinal is what tells two repeated `--lint` commands (or two policy directories of the same tool) apart. The built-in adapters label their findings for you; a `--lint` command's stdout is passed through **verbatim**, so its findings carry an identity only if the adapter adds one — argocdf exports the identity to every command as **`ARGOCDF_LINT_ID`** for exactly that (the reference adapters in this README interpolate it). An adapter that ignores it still works; its findings are simply unlabelled. The command text is deliberately *not* echoed back into the report: the identity already says which `--lint` failed, so nothing you pass in a command — credentials included — reaches a PR comment. Tools like kyverno and conftest exit non-zero when policies fail (normal operation), so end the pipeline in an adapter — typically `jq`, which also normalizes any tool's output to line-per-finding.
