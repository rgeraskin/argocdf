...
- Fix: the render cache no longer answers questions about credentials it never used
  Keys now include the `--repo-creds` source and the repository name-to-URL mappings it provides (never the credentials themselves). Two problems, one bump. Switching modes to VERIFY something - render with `local`, re-run with `cluster` to check ArgoCD's credentials work - used to be answered out of the first run's cache, so a repository ArgoCD could not reach only failed after merge. And a chart dependency naming its repository by alias (`repository: "@myrepo"`) resolves through those mappings, so pointing a name elsewhere changed what rendered while the cache served the old manifests. The chart cache is scoped by credential source too, so the switch actually re-fetches instead of serving a chart downloaded under the other source - one re-download per mode, which is the price of the check being real. Mind that the chart cache has no GC (only the render cache is bounded), so `charts/` grows per mode until `argocdf cache clean`. Within a single mode an already-downloaded chart is still served without contacting the registry. Existing entries are invalidated once by the `rendercache-v4` bump.
