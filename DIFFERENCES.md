...
| **`$values` files once declared**     | not covered by the declaration either — the docs note external values files miss out                     | ✅ same: the declaration replaces argocdf's `$ref` file matching, so declare those paths         |
