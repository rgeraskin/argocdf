...
| **Chart revision**   | resolved per source and passed to the render: an exact version as declared, a semver CONSTRAINT resolved against the index/tag list. `HEAD` and empty are neither, so the render FAILS | ✅ same resolution, through ArgoCD's own `util/versions` — it pins the pull and feeds `ARGOCD_APP_REVISION*`. The one deviation is at the edge upstream rejects: `HEAD`/empty means "latest" here (helm picks it), and the build env reports the DECLARED revision rather than failing the render |
...
| **Ref materialization**       | LAZY — a ref repository is cloned only when a `$ref/…` value file names it | EAGER (GenerateManifests never clones), except for `oci://` and `chart:` ref sources, which are registered but not materialized: `git clone` cannot fetch either, and a referenced one then fails in manifest generation exactly as upstream's does |
| **External source revision**  | the external repository's resolved commit                                  | ✅ same — the clone's commit, not the diffed repository's                                                                                                              |
...
| **`ARGOCD_APP_REVISION`**     | the resolved digest (for charts, the resolved chart version)                       | ✅ same — the revision handed to `GenerateManifests` is resolved PER SOURCE: digest, chart version, or git commit                |
...
| **`path` is mandatory**       | `validateSourcePermissions` requires `path` OR `chart` on every source, with NO `oci://` carve-out — an artifact source without one is an `InvalidSpecError` the controller never renders | ✅ same rule, mirrored in `cluster.ValidateSourceSpec` and applied per SIDE before each render: such an application is reported as an error, never as a diff. `path: .` is the artifact root, and what every e2e fixture uses |

That last row is worth stating twice, because the shape looks complete: `spec.source.path` or `spec.source.chart` is mandatory on EVERY source, ArgoCD applies that check to `oci://` sources too, and `repoURL` + `targetRevision` alone therefore produces an `InvalidSpecError` — the controller reports it on the resource and never renders the application. Give an artifact source `path: .` unless it renders a subdirectory of the artifact.

argocdf refuses that spec too, which is the one place it validates a spec at all. It renders by calling `GenerateManifests` directly, BELOW the controller's validation, so it used to diff such an application happily and report "No changes" — the worst available answer, since the application does not exist in the cluster. `cluster.ValidateSourceSpec` mirrors upstream's source rules (`util/argo.validateSourcePermissions`, including the `sourceHydrator` branch that replaces them) and runs at the top of each side's render, before the render cache: the application's report line carries ArgoCD's own message and the side whose spec is refused, and every other application still diffs. Upstream's exported caller is deliberately NOT used — it demands an `AppProject` and a `db.ArgoDB` and enforces project and destination policy argocdf does not (§14) — so the copy is pinned against the real thing by `TestValidateSourceSpecMatchesUpstream`, which runs every shape it knows past `argo.ValidatePermissions` in both directions: one argocdf refuses must still be refused upstream, and one it accepts must draw no condition, so the mirror cannot silently outlive either a relaxed or a newly added rule. Project scoping, destination permissions and the remaining spec rules are still unchecked here.
