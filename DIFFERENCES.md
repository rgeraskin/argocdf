...
| **`path` is mandatory**       | `validateSourcePermissions` requires `path` OR `chart` on every source, with NO `oci://` carve-out — an artifact source without one is an `InvalidSpecError` the controller never renders | ❌ not validated: argocdf renders such a source anyway, so an application ArgoCD has refused still appears in a report. `path: .` is the artifact root, and what every e2e fixture uses |

That last row is the one shape that renders here and cannot exist in ArgoCD, so it is worth stating twice: `spec.source.path` or `spec.source.chart` is mandatory on EVERY source, ArgoCD applies that check to `oci://` sources too, and `repoURL` + `targetRevision` alone therefore produces an `InvalidSpecError` — the controller reports it and never renders the application. argocdf runs no spec validation at all (it renders what the cluster holds), so it will happily diff an application the controller has already refused. Give an artifact source `path: .` unless it renders a subdirectory of the artifact.
